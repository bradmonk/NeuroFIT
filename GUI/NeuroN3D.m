function [varargout] = NeuroN3D(varargin)
...
oDUBs = iDUBs; % Save an original unprocessed "iDUBs" image stack
...
for mm = 1:numel(iDUBs)
    ...
    for nn = 1:numel(iD)
        x = iD(nn);
        if maxIMG > 1
            iD(nn) = lintrans(x,minIMG,maxIMG,0,1);
        else
            iD(nn) = lintrans(x,minIMG,1,0,1);
        end
    end
...
%  ------ PLOT IMAGE STACK  -------
    ...
    for mm = 1:numel(iDUBs)
        ...
        imagesc(iDUBs{mm})
        title(ImageFiles{mm},'Interpreter','none')
        drawnow

        pause(.02)
        disp(strcat('iDUBs{',num2str(mm),'}  :: ' ,ImageFiles{mm}))
        pause(.02)
...
%% -- MESH SURFACE PLOT
    ...
    for mm = 1:numel(iDUBs)
        ...
        imax(mm) = max(max(iDUBs{mm}));
        imean(mm) = mean(mean(iDUBs{mm}));
        
    end

MaxMean = mean(imax)
PctMean = MaxMean / mean(imean)

%%
    for mm = 1:numel(iDUBs)

        hfmx = (imax(mm) * .05) + (MaxMean * .01);
...
%         hfmx = (imax(mm) * .05) + (MaxMean * .01);
% 
%         mesh(double(iDUBs{mm} > hfmx))
%             xlim([0 550]); ylim([0 550]); %zlim([0 .05])
%             view([-178 90])
%             drawnow
%         
%     end
...
%% %-------  CREATE 3D SURFACE MESH OF NEURON STACK

%-------
fig1 = figure(1);
set(fig1,'OuterPosition',[100  200  1000  800],'Color',[1,1,1])
hax1 = axes('Position',[.07 .07 .9 .9]);
...
set(fig1,'OuterPosition',[100  200  1000  800],'Color',[1,1,1])
    ...
    hax1 = axes('Position',[.05 .05 .95 .95]);
           axis vis3d

hiso = patch(isosurface(IMG,.001),...
    'FaceColor',[.9,.4,.8],...
    'EdgeColor',[.2,.4,.8],...
    'FaceLighting','gouraud','EdgeAlpha',.1);
   isonormals(IMG,hiso)

az = -220; el = 5; view([az el]); drawnow;

for tt = 1:100
    view([az el]);  az=az+sqrt(abs(az))/el; el=el+.51;
    drawnow;
end
